...
{
private:
    std::unique_ptr<AdaptiveHumanTracker::RegisterPersonCommandExecutorInputData> m_inputData;
...
public:
    ...
    {
        m_inputData.reset(new AdaptiveHumanTracker::RegisterPersonCommandExecutorInputData(*other.m_inputData));
        ...
        bool openCVHIGHGUIDisplay = true)
    {
        ...
        //m_inputData = std::make_unique< AdaptiveHumanTracker::RegisterPersonCommandExecutorInputData>();
        m_inputData = std::unique_ptr< AdaptiveHumanTracker::RegisterPersonCommandExecutorInputData>(new AdaptiveHumanTracker::RegisterPersonCommandExecutorInputData());
    ...
    {
        m_inputData.reset(new AdaptiveHumanTracker::RegisterPersonCommandExecutorInputData(*rhs.m_inputData));
    ...
    bool RunExecute()
	{
		...
		if (m_inputData->registrationMode == AdaptiveHumanTracker::FaceRegistrationMode::ONLINE) {
...
void RegisterPersonCommandExecutor::RegisterHostUIUpdateEventHandler()
{
    ...
    AdaptiveHumanTracker::EventHandler<AdaptiveHumanTracker::Event, cv::Mat>::Register<RegisterPersonCommandExecutor>(eventForSubscription, *this);

    
}

void RegisterPersonCommandExecutor::UnRegisterHostUIUpdateEventHandler()
{
    // Initialize all the objects for event handlers
    AdaptiveHumanTracker::Event eventForSubscription =
        AdaptiveHumanTracker::Event::FACE_REGISTRATION;
    AdaptiveHumanTracker::EventHandler<AdaptiveHumanTracker::Event, cv::Mat>::UnRegister<RegisterPersonCommandExecutor>(eventForSubscription, *this);

    
}
RegisterPersonCommandExecutor::RegisterPersonCommandExecutor(std::function<void(cv::Mat*)>& uiUpdateHandlerSTDFunction, 
...
RegisterPersonCommandExecutor::RegisterPersonCommandExecutor(UIUpdateCallbackFuncPtr uiUpdateHandler, 
    bool openCVHighGUIDisplay) :
...
//    RegisterPersonCommandExecutor::RegisterPersonCommandExecutor(rhs);
//    //m_OpenCVHighGUIDisplay = rhs.m_OpenCVHighGUIDisplay;
//    //RegisterHostUIUpdateEventHandler();
//    return *this;
//}
RegisterPersonCommandExecutor::RegisterPersonCommandExecutor(const std::string& modelDirectoryPath,
    ...
    m_OpenCVHighGUIDisplay(openCVHighGUIDisplay)
{
    m_inputData = { modelDirectoryPath, dataDirectoryPath, imageDirectoryPath, personName, registrationMode };
    //Private implementor
...
void RegisterPersonCommandExecutor::OnEventNotified(cv::Mat& openCVImage)
{
...
void RegisterPersonCommandExecutor::Terminate()
{
    if (m_implementor )
        m_implementor->Terminate();
}
std::unique_ptr<NullData> RegisterPersonCommandExecutor::RunExecute(AdaptiveHumanTracker::RegisterPersonCommandExecutorInputData& inputData)
{
