...
FaceDetection::FaceDetection(std::string model_path, string device)
{
    ...
    device_ = device;
    // set anchor 
    create_anchor(m_anchor, OPENVINO_DET_INPUT_WIDTH, OPENVINO_DET_INPUT_HEIGHT); 
    // for(size_t i=0; i<m_anchor.size(); i++)
    // {
    //     printf("Ceated Anchor info: %.2f, %.2f,%.2f,%.2f\n", m_anchor[i].cx, 
    //     m_anchor[i].cy, m_anchor[i].sx, m_anchor[i].sy); 
...
//    printf("Face detection Line93\n"); 
 //   printf("fp_image size: %d, %d\n", fp_img.cols, fp_img.rows); 
  //  printf("mean size: %d, %d\n", mean_.cols, mean_.rows); 
    cv::subtract(fp_img, mean_, fp_img);
 //   printf("Face detection Line95\n"); 
    cv::split(fp_img, input_channels);
  //  printf("Face detection Line99\n"); 
    //openvino infer
    infer_request.Infer();
